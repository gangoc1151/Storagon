...
export function* loginMetaMask({ payload }) {

    console.log('===saga loginMetaMask', payload);
    let response = yield call(fetchApiLogin, 'post', 'clapi/web3/web3auth/', {...payload})
    console.log({ loginMetaMask: response })
    if (response && response.token) {
        Token.setToken(response.token);
...
export function* getKeySign() {

    console.log('===saga getKeySign');
    let response = yield call(fetchApi, 'get', 'login_api/')
    console.log({ profile: response })
    if (response) {
        yield put({
            type: actions.GET_KEY_SIGN_SUCCESS,
            data: response
        });
    }
...
export default function* rootSaga() {
    yield [
        ...
        yield takeLatest(actions.LOGIN_METAMASK, loginMetaMask),
        yield takeLatest(actions.GET_KEY_SIGN, getKeySign),
